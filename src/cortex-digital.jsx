...
function getRandomSuggestions(n = 4) {
  return shuffleArray(ALL_SUGGESTIONS).slice(0, n);
}
...
const defaultBrain = {
  ...
  semantic: [],
  patterns: [],
  procedural: { format: "conciso", lang: "pt", level: "médio" },
  sessions: 0,
  lastReflect: null,
};
const defaultKeys = {
...
function getDevPin() {
  return localStorage.getItem(DEV_PIN_KEY) || "3004";
}
...
async function safeGet(key, fallback) {
  try {
    ...
    if (local) return JSON.parse(local) ?? fallback;
  } catch {}
  try {
    const r = await window.storage.get(key);
    if (!r?.value) return fallback;
    return JSON.parse(r.value) ?? fallback;
  } catch {
    return fallback;
  }
}
...
function normBrain(r) {
  ...
  return {
    ...
    procedural:
      r.procedural && typeof r.procedural === "object"
        ? { ...defaultBrain.procedural, ...r.procedural }
        : { ...defaultBrain.procedural },
...
function buildMem(b) {
  const p = [];
  if (b.semantic.length)
    p.push(
      "FACTS:\n" +
        b.semantic
          .slice(-15)
          .map((s) => `• [${s.tipo}] ${s.descricao}`)
          .join("\n"),
    );
  if (b.episodic.length)
    p.push(
      "PAST:\n" +
        b.episodic
          .slice(-5)
...
function selectUsedMem(brain, q) {
  ...
  const scored = brain.semantic.map((s) => ({
    text: `[${s.tipo}] ${s.descricao}`,
    score: ql
      .split(" ")
      .filter((w) => w.length > 3 && s.descricao.toLowerCase().includes(w))
      .length,
  }));
  scored.sort((a, b) => b.score - a.score);
  return [...scored.slice(0, 3).map((s) => s.text), ...brain.episodic.slice(-2)]
    .filter(Boolean)
    .slice(0, 5);
}
function heuristicDecision(q) {
  const s = q.toLowerCase();
  if (/c[oó]digo|code|programar|script|bug|erro|implementar/.test(s))
    return "Pergunta técnica → priorizei DeepSeek + Córtex.";
  if (/hoje|atual|notícia|recente|2025|2026|mercado|preço/.test(s))
    return "Pergunta factual → priorizei Perplexity + Grok.";
  if (/porquê|filosofia|contexto|história|tendência|futuro/.test(s))
    return "Pergunta conceptual → priorizei Gemini + Nemotron.";
  if (/\beu\b|\bmeu\b|\bminha\b|pessoal|objetivo|estudo|curso/.test(s))
    return "Pergunta pessoal → memória semântica + Córtex.";
...
function seedToMem(text, tipo) {
  return text
    ...
    .slice(0, 3)
    .map((s) => ({ tipo, descricao: s.slice(0, 160), importancia: "alta" }));
}
function safeParseReflect(raw) {
  try {
    const p = JSON.parse(raw.replace(/```json|```/g, "").trim());
    return {
      new_semantic: Array.isArray(p.new_semantic) ? p.new_semantic : [],
      new_patterns: Array.isArray(p.new_patterns) ? p.new_patterns : [],
      procedural_update:
        p.procedural_update && typeof p.procedural_update === "object"
          ? p.procedural_update
          : {},
      session_summary:
...
function detectLang(q) {
  const pt =
    /[áàãâéêíóôõúüçÁÀÃÂÉÊÍÓÔÕÚÜÇ]|\b(que|como|qual|quais|onde|quando|porque|isto|isso|sobre|para|com|uma)\b/i;
  return pt.test(q)
...
const P = {
  grok: (m, q) =>
    `You are GROK — facts expert. Give concrete, precise data. No intro. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  gemini: (m, q) =>
    `You are GEMINI — systems thinker. Find patterns and big-picture insights. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  perp: (m, q) =>
    `You are PERPLEXITY — current info. Provide recent, accurate, sourced info. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  genspark: (m, q) =>
    `You are GENSPARK — creative synthesis. Novel angles and unexpected solutions. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  manus: (m, q) =>
    `You are MANUS — AUTONOMOUS AGENT: step-by-step execution planner.\nMEMORY:\n${m}\nQUERY: "${q}"\nAgentic steps, tools, actions. Max 120 words. No intro. Same language.`,
  openai: (m, q) =>
    `You are a reasoning expert. Chain-of-thought, structured analysis. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  deepseek: (m, q) =>
    `You are DEEPSEEK — code & logic expert. For code use markdown fences. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  llama: (m, q) =>
    `You are LLAMA — broad community knowledge, practical open-source experience. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  mistral: (m, q) =>
    `You are MISTRAL — fast and precise. No padding. Memory:\n${m}\nQuestion: ${q}\nMax 80w. ${detectLang(q)}`,
  nemotron: (m, q) =>
    `You are NEMOTRON — scientific rigor. Evidence-based, cite mechanisms. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  ollama_codigo: (m, q) =>
    `Local coding assistant. Give clean working code with brief explanation. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  ollama_debug: (m, q) =>
    `Local debug expert. Find root cause, give exact fix. Memory:\n${m}\nQuestion: ${q}\nMax 100w. ${detectLang(q)}`,
  compress: (msgs) =>
    `Summarize this conversation history in 1 compact paragraph (max 80 words). Keep facts, decisions, and key context. No intro.\n\n${msgs.join("\n")}`,
  cortex: (m, q, lobes) =>
    `
...
`.trim(),
  ...
  judge: (q, lobeResults) =>
    `You are the judge of an 11-lobe AI council.
Question: "${q}"
Lobe responses:
...
Write ONE sentence (max 80 words) in Portuguese explaining which lobes were most useful and why. No lists.`.trim(),
  ...
  reflect: (buf, mem) =>
    `Analisa esta conversa e devolve APENAS JSON válido, sem markdown.
...
function classifyQuery(q) {
  try {
    ...
    if (
      words < 8 &&
      /^(olá|oi|bom dia|boa tarde|obrigad|ok|sim|não|certo|fixe|hey|hi|hello)/.test(
        s,
      )
    )
      return "SIMPLE";
    ...
    const complexPatterns =
      /porquê|porque\s|como\s|compara|analisa|debate|diferença|vantagens?|desvantagens?|explica|impacto|estratégia|avalia|crítica|pros\s|contras\s|trade.?off/i;
...
function routerDecide(query) {
  ...
  const isCode =
    /código|code|programar|script|bug|erro|implementar|react|js|python|jsx/.test(
      q,
    );
...
async function callGrok(sys, msg, key) {
  const r = await fetchWithTimeout("/api/grok/v1/chat/completions", {
    method: "POST",
    headers: {
...
function Markdown({ text, color, faint }) {
  ...
  while (i < lines.length) {
    ...
    if (/^\d+\. /.test(line)) {
      ...
      out.push(
        <div key={i} style={{ display: "flex", gap: 6, marginTop: 2 }}>
          <span style={{ color: faint, flexShrink: 0, minWidth: 14 }}>
            {n}.
          </span>
          <span>{iFmt(line.replace(/^\d+\. /, ""))}</span>
        </div>,
      );
      i++;
      continue;
    ...
    out.push(
      <div key={i} style={{ marginTop: 1, lineHeight: 1.75 }}>
        {iFmt(line)}
      </div>,
    );
    i++;
...
function iFmt(text) {
  ...
  while ((m = rx.exec(text)) !== null) {
    ...
    if (m[2] !== undefined)
      parts.push(
        <b key={m.index} style={{ fontWeight: 700 }}>
          {m[2]}
        </b>,
      );
          ...
          .map((c, i) => (
            <div
              ...
              style={{
                ...
                borderRadius: "50%",
                background: c,
                animation: `orb 1.4s ${i * 0.18}s ease-in-out infinite`,
              }}
            />
          ))}
      </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 style={{ margin: 0, fontSize: 13, fontWeight: 800, color: T.tx }}>
            {title}
          </h3>
          <button
            onClick={onClose}
  ...
  return (
    <div style={{ background: T.s2, borderRadius: 11, padding: "9px 11px" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div
            style={{
              ...
              height: 6,
              borderRadius: "50%",
              background: active ? api.color : T.b2,
              boxShadow: active ? `0 0 6px ${api.color}` : "none",
              transition: "all 0.3s",
            }}
          />
        ...
        <span
          style={{
            ...
            padding: "2px 7px",
            borderRadius: 20,
            background: active ? `${api.color}22` : T.s1,
            border: `1px solid ${active ? `${api.color}44` : T.b1}`,
      ...
      <div style={{ display: "flex", gap: 5 }}>
        <input
          ...
          style={{
            flex: 1,
            background: T.s1,
            border: `1px solid ${active ? `${api.color}55` : T.b1}`,
            borderRadius: 8,
            padding: "6px 9px",
            color: T.tx,
            fontSize: 10,
      ...
      <div style={{ display: "flex", gap: 5, marginTop: 5 }}>
        <button
          onClick={testKey}
          disabled={draft.trim().length < 10 || status === "testing"}
          style={{
            flex: 1,
            background: T.s1,
            border: `1px solid ${T.b1}`,
            ...
            color:
              status === "ok" ? "#10b981" : status === "err" ? "#ef4444" : T.ts,
          }}
        >
          {status === "testing"
            ? "A testar..."
        ...
        <button
          ...
          style={{
            flex: 1,
            background: dirty ? `${api.color}22` : T.s1,
            border: `1px solid ${dirty ? `${api.color}66` : T.b1}`,
            borderRadius: 8,
            padding: "5px 0",
...
export default function Cortex() {
  ...
  const [modelsOn, setModelsOn] = useState(
  ...
  const [pendingImage, setPendingImage] = useState(null); // {base64,mime,preview,name}
  ...
  const hG = keys.grok?.trim().length > 10,
    hGm = keys.gemini?.trim().length > 10;
  const hP = keys.perp?.trim().length > 10,
    hC = keys.claude?.trim().length > 10;
  const hO = keys.openai?.trim().length > 10,
    hD = keys.deepseek?.trim().length > 10;
  const hL = keys.llama?.trim().length > 10,
    hM = keys.mistral?.trim().length > 10;
  ...
  useEffect(() => {
    if (atBottom) botRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [msgs, phase]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (page !== "keys" && !DEV_MODE) setDevUnlocked(false);
  }, [page]);
  ...
  async function load() {
    try {
      ...
      const k =
        (await safeGet("cortex-keys-global", null)) ||
        (await safeGet(MV + "-keys", defaultKeys));
      ...
      setModelsOn(
        mo && typeof mo === "object"
          ? mo
          : Object.fromEntries(MODELS.map((x) => [x.id, true])),
      );
      // setCompTasks(Array.isArray(ct)?ct:[]); // REMOVIDO v12
    } catch (e) {
      toast("Falha ao carregar dados — reset para defaults");
    }
    setLoaded(true);
  }
  ...
  function deleteConv(convId, e) {
    ...
    setConversations((prev) => {
      const u = prev.filter((c) => c.id !== convId);
      saveConvs(u);
      return u;
    });
  ...
  function autoSaveConv(currentMsgs, convId) {
    ...
    const title =
      currentMsgs.find((m) => m.role === "user")?.content?.slice(0, 45) ||
      "Conversa";
  ...
  async function invoke(id, sys, msg) {
    ...
    try {
      ...
      if (id === "ollama_codigo") {
        try {
          const r = ok(await callOllama(sys, msg, "codigo"));
          cacheSet(id, msg, r);
          return r;
        } catch (e) {
      ...
      if (id === "ollama_debug") {
        try {
          const r = ok(await callOllama(sys, msg, "debug"));
          cacheSet(id, msg, r);
          return r;
        } catch (e) {
...
const routedIds = routerDecide(qWithImg);
    ...
    const results = await Promise.allSettled(
      councilLobes.map((l) =>
        invoke(l.id, P[l.id]?.(mem, qFinal) || `Answer: ${qFinal}`, qFinal),
      ),
    );
    const lobeResults = councilLobes.map((l, i) => {
      const r =
        results[i].status === "fulfilled"
          ? results[i].value
          : {
              result: `Tempo esgotado ou serviço indisponível`,
              model: "?",
              real: false,
            };
      const isErr =
    ...
    let structured;
    try {
      const validLobes = lobeResults.filter(
        (l) => !l.isErr && l.result?.length > 10,
      );
      if (hC || hP)
        cR = await callClaude(
        ...
        const validFb = lobeResults.filter(
          (l) => !l.isErr && l.result?.length > 10,
        );
        cR =
          validFb.length > 0
    ...
    const council = Object.fromEntries(
      lobeResults.map((l) => [l.id, l.result]),
    );
    ...
    const buf2 = [
      ...newBuf,
      `BRAIN: ${(structured?.final || cR || "").trim()}`,
    ];
    setBuf(buf2);
    ...
    if (buf2.length >= MAX_BUF && nb.sessions >= 1) {
      ...
      try {
        ...
        nb = {
          ...nb,
          semantic: [...nb.semantic, ...(ext.new_semantic || [])].slice(
            -MAX_SEMANTIC,
          ),
          patterns: [
            ...new Set([...nb.patterns, ...(ext.new_patterns || [])]),
          ].slice(-MAX_PATTERNS),
        ...
        reflexOk = !!(
          ext.new_semantic?.length ||
          ext.new_patterns?.length ||
          ext.session_summary
        );
      } catch {
  ...
  function applySeed() {
    const entries = [
      ...seedToMem(seedP, "facto"),
      ...seedToMem(seedC, "facto"),
      ...seedToMem(seedO, "objetivo"),
    ];
    if (!entries.length) return;
    const nb = {
      ...brain,
      semantic: [...brain.semantic, ...entries].slice(-MAX_SEMANTIC),
      episodic: [
        ...brain.episodic,
        "Configuração inicial (seed manual).",
      ].slice(-MAX_EPISODIC),
    };
    setBrain(nb);
    saveBrain(nb);
  ...
  function doImport() {
    ...
    try {
      ...
      if (!Array.isArray(raw.semantic) || !Array.isArray(raw.episodic))
        throw new Error("Formato inválido.");
      setBrain(normBrain(raw));
      saveBrain(normBrain(raw));
      setBuf([]);
      setShowImport(false);
      setImportTxt("");
  ...
  const phases = {
    council: {
      label: `${LOBES.filter((l) => l.id !== "claude" && modelsOn[l.id] !== false).length} lobos a analisar...`,
      color: "#a78bfa",
      pct: "50%",
    },
    cortex: {
      label: "Claude Opus 4.6 a sintetizar...",
      color: AC.claude,
      pct: "88%",
    },
    reflex: {
      label: "Reflexão subconsciente...",
      color: AC.reflex,
      pct: "100%",
    },
  };
    ...
    transition:
      ...
      {showGuide && (
        ...
        >
          ...
          >
            <p>
              <b style={{ color: T.tx }}>Conselho de 11 Lobos</b>
              <br />
              Cada pergunta é analisada em paralelo por até 11 modelos. Claude
              Opus 4.6 age como juiz e sintetiza a melhor resposta final.
            </p>
            <div
              style={{
                display: "grid",
            ...
            >
              {[
                ["◉ Grok", "Factos empíricos", "grok-3"],
                ["◈ Gemini", "Contexto amplo", "gemini-2.5-flash"],
                ["◇ Perplexity", "Web atual", "sonar-pro"],
                ["◎ Genspark", "Síntese multi-AI", "simulado"],
                ["◍ Manus", "Agente autónomo", "via Claude"],
                ["○ OpenAI", "Raciocínio", "gpt-4o"],
                ["◐ DeepSeek", "Código/Lógica", "deepseek-chat"],
                ["◑ Llama", "Open source", "llama-4-scout"],
                ["◒ Mistral", "Velocidade", "mistral-large"],
                ["◓ Nemotron", "Ciência", "nemotron-4-340b"],
                ["◆ Claude", "Juiz final", "claude-opus-4-6"],
              ].map(([l, d, v], i) => (
                <>
                  <span key={i + "a"} style={{ fontWeight: 700, color: T.tx }}>
                    {l}
                  </span>
                  <span key={i + "b"}>{d}</span>
                  <span
            ...
            <p>
              <b style={{ color: T.tx }}>Córtex</b>
              <br />O Claude Opus 4.6 atua como juiz do conselho e sintetiza a
              resposta final com base nos lobes ativos.
            </p>
            <p>
              <b style={{ color: T.tx }}>Memória</b>
              <br />
              Usa "Seed" para dar contexto inicial. A cada {MAX_BUF} trocas o
              sistema consolida memórias automaticamente.
            </p>
            <p style={{ color: T.tf, fontSize: 10 }}>
              Shift+Enter = nova linha · 21 temas · Histórico sem limite · ↺
              regenerar · ↓ exportar · 📤 partilhar
            </p>
          </div>
        </Modal>
      )}
      ...
      {showExport && (
        ...
        >
          <p style={{ fontSize: 11, color: T.ts, marginBottom: 7 }}>
            JSON do teu cérebro:
          </p>
          <textarea
            readOnly
            value={JSON.stringify(normBrain(brain), null, 2)}
          ...
          <button
            onClick={() =>
              navigator.clipboard
                ?.writeText(JSON.stringify(normBrain(brain), null, 2))
                .then(() => toast("Copiado!", "success"))
            }
            style={{ ...btn(T, AC.claude), marginTop: 7, width: "100%" }}
          >
            📋 Copiar
          </button>
        </Modal>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
            {[
              [
                ...
                "Ex: estudante de Automação na ESTCB, gaming, IA...",
                seedP,
                setSeedP,
              ],
              [
                "Contexto atual",
                "Ex: 2º ano, projeto de automação de linha de produção...",
                seedC,
                setSeedC,
              ],
              [
                "Objetivos principais",
                "Ex: terminar curso, estágio em automação, melhorar Python...",
                seedO,
                setSeedO,
              ],
            ].map(([lbl, ph, val, set]) => (
              ...
              >
                <label style={{ fontSize: 11, fontWeight: 600, color: T.ts }}>
                  {lbl}
                </label>
                <textarea
                  value={val}
            ...
            <button
              onClick={applySeed}
              style={{ ...btn(T, AC.claude), width: "100%" }}
            >
              💾 Guardar seed de memória
            </button>
            <p style={{ fontSize: 9, color: T.tf, margin: 0 }}>
              Frases com menos de 20 chars são ignoradas. Regista em Episódica.
            </p>
          </div>
        </Modal>
      )}
          ...
          >
            {Object.entries(THEMES).map(([key, th]) => (
              <button
                ...
                style={{
                  background: theme === key ? th.s2 : "transparent",
                  border: `2px solid ${theme === key ? AC.claude : th.b1}`,
                  borderRadius: 13,
                  padding: "9px 11px",
                  cursor: "pointer",
                  fontFamily: "inherit",
                  ...
                  boxShadow: theme === key ? `0 0 12px ${AC.claude}44` : "none",
                }}
              >
                ...
                <div style={{ textAlign: "left" }}>
                  <div
                    style={{
                      fontSize: 11,
                      fontWeight: 700,
                      color: theme === key ? th.tx : T.ts,
                    }}
                  >
                    {th.name}
                  </div>
                  <div style={{ display: "flex", gap: 3, marginTop: 3 }}>
                    {[th.bg, th.s1, AC.claude, th.tx].map((c, i) => (
                      <div
                        key={i}
                        style={{
                          width: 10,
      ...
      {showModels && (
        ...
        >
          <p style={{ fontSize: 11, color: T.ts, marginBottom: 8 }}>
            Desactiva modelos que não precisas. Os desactivados são simulados
            por Claude.
          </p>
              ...
              >
                <div
                  style={{
                    ...
                    height: 7,
                    borderRadius: "50%",
                    background: modelsOn[m.id] !== false ? m.color : "#666",
                    flexShrink: 0,
                  }}
                />
                <div style={{ flex: 1 }}>
                ...
                <Toggle
                  on={modelsOn[m.id] !== false}
                  onChange={(v) => {
                    const ne = { ...modelsOn, [m.id]: v };
                    setModelsOn(ne);
                    saveModels(ne);
                  }}
                  color={m.color}
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 7, color: T.tf, letterSpacing: 1 }}>
              11 Lobos · Council · Claude Opus 4.6
            </div>
          </div>
        </div>
            ...
            {LOBES.slice(0, 10)
              ...
              .map((l) => {
                ...
                return (
                  <div
                    key={l.id}
                    title={`${l.label}: ${hasKey ? "Real" : "Simulado"}`}
                    style={{
                      padding: "2px 5px",
                      borderRadius: 16,
                      border: `1px solid ${hasKey ? l.color + "44" : T.b1}`,
                      background: hasKey ? l.color + "12" : "transparent",
                      display: "flex",
                      alignItems: "center",
                      gap: 2,
                    }}
                  >
                    <div
                      style={{
                        ...
                        height: 4,
                        borderRadius: "50%",
                        background: hasKey ? l.color : T.tf,
                      }}
                    />
                    <span
                      style={{
        ...
        {!isMobile && (
          <>
            <button
              ...
              style={{ ...navBtn(T), minWidth: 44, minHeight: 44 }}
              title="Modelos"
            >
              ◈
            </button>
      ...
      {/* Progress */}
      {phase && (
        <div style={{ height: 2, background: T.b2, flexShrink: 0 }}>
          <div
            style={{
              height: "100%",
              width: cur?.pct || "0%",
              background: `linear-gradient(90deg,${cur?.color}88,${cur?.color})`,
        ...
        <>
          <>
            ...
            >
              ...
              >
                <span style={{ fontSize: 11, fontWeight: 700, color: T.tx }}>
                  Histórico
                </span>
                <button
                  onClick={newChat}
                ...
                {conversations.length === 0 ? (
                  ...
                  conversations.map((conv) => (
                    <div
                      ...
                      style={{
                        background:
                          conv.id === currentConvId ? `${AC.claude}18` : T.s2,
                        border: `1px solid ${conv.id === currentConvId ? AC.claude + "44" : T.b1}`,
                        borderRadius: 8,
                        padding: "7px 9px",
                        cursor: "pointer",
                        display: "flex",
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 8, color: T.ts, marginTop: 1 }}>
                          {conv.msgs?.filter((m) => m.role === "user").length}{" "}
                          msg ·{" "}
          ...
          {DEV_MODE && (
            ...
            >
              {LOBES.map((l, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 5,
                          borderRadius: "50%",
                          background: active || done ? l.color : "#444",
                      ...
                      <span
                        style={{
                          color: active || done ? l.color : T.ts,
                    ...
                    {i < LOBES.length - 1 && (
                      <span style={{ color: T.ts, opacity: 0.15 }}>·</span>
                    )}
                  </div>
                );
              })}
              ...
              >
                <span>
                  <b style={{ color: AC.claude }}>{brain.semantic.length}</b>{" "}
                  fac
                </span>
                <span>
                  <b style={{ color: AC.gemini }}>{brain.sessions}</b> sess
                </span>
                <span>
                  <b style={{ color: AC.grok }}>
                    {buf.length}/{MAX_BUF}
                  </b>{" "}
                  buf
                </span>
                ...
                <span title="Respostas em cache">
                  <b style={{ color: AC.perp }}>{_cache.size}</b>⚡
                </span>
              </div>
            </div>
          )}
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  boxShadow: fabOpen
                    ? `0 10px 28px ${AC.claude}55`
                    : "0 10px 28px #00000044",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  transform: fabOpen ? "rotate(90deg)" : "rotate(0deg)",
                  transition:
                    "transform 300ms cubic-bezier(0.4,0,0.2,1), background 300ms cubic-bezier(0.4,0,0.2,1), box-shadow 300ms cubic-bezier(0.4,0,0.2,1), color 300ms cubic-bezier(0.4,0,0.2,1), border-color 300ms cubic-bezier(0.4,0,0.2,1)",
                }}
              >
                ⚙
              </button>
          ...
          >
            ...
            {msgs.length === 0 ? (
              ...
              >
                ...
                >
                  <div
                    style={{
                      ...
                      inset: 0,
                      borderRadius: "50%",
                      background: `radial-gradient(circle at 35% 30%, ${AC.claude}cc, ${AC.claude}33, transparent)`,
                      boxShadow: `0 0 24px ${AC.claude}44`,
                  ...
                  {[AC.grok, AC.gemini, AC.perp, AC.manus].map((c, i) => (
                    <div
                      ...
                      style={{
                        ...
                        left: "50%",
                        transformOrigin: "0 0",
                        animation: `orbit${i} ${2.2 + i * 0.45}s linear infinite`,
                      }}
                    />
                  ))}
                </div>
                ...
                >
                  Conselho de{" "}
                  <span style={{ color: AC.claude, fontWeight: 600 }}>
                    11 lobos
                  </span>{" "}
                  · Juiz{" "}
                  <span style={{ color: AC.claude, fontWeight: 600 }}>
                ...
                >
                  {[
                    {
                      icon: "◉",
                      text: "Explica memória vetorial em sistemas de IA",
                      color: AC.grok,
                    },
                    {
                      icon: "◈",
                      text: "Como criar um agente IA com LangChain?",
                      color: AC.gemini,
                    },
                    {
                      icon: "◐",
                      text: "Diferenças práticas entre os principais LLMs",
                      color: AC.deepseek,
                    },
                    {
                      icon: "◇",
                      text: "O que é RAG e como funciona na prática?",
                      color: AC.perp,
                    },
                  ].map((s, i) => (
                    <button
                    ...
                    >
                      <span
                        style={{ color: s.color, fontSize: 14, flexShrink: 0 }}
                      >
                        {s.icon}
                      </span>
                ...
                >
                  {brain.semantic.length > 0 ? (
                    <span>
                      🧠 {brain.semantic.length} factos · {brain.sessions}{" "}
                      sessões
                    </span>
                  ) : (
                    <button
                    ...
                    {m.role === "user" ? (
                      ...
                      <div
                        style={{
                          ...
                          animation:
                            "fadeIn 250ms cubic-bezier(0.4,0,0.2,1), lobePop 250ms cubic-bezier(0.4,0,0.2,1)",
                        }}
                      >
                          ...
                          >
                            ...
                            <button
                              onClick={async () => {
                                ...
                                const consensus = m.structured?.consensus
                                  ?.length
                                  ? "\n\nConsenso:\n- " +
                                    m.structured.consensus.join("\n- ")
                                  : "";
                                const divergence = m.structured?.divergence
                                  ?.length
                                  ? "\n\nDivergências:\n- " +
                                    m.structured.divergence.join("\n- ")
                                  : "";
                                const nextActions = m.structured?.nextActions
                                  ?.length
                                  ? "\n\nPróximos passos:\n" +
                                ...
                                if (navigator.share && isMobile) {
                                  ...
                                  toast(
                                    "Resposta copiada para partilha",
                                    "success",
                                  );
                                }
                              }}
                            ...
                            {m.lobeResults?.length > 0 && (
                              <button
                                ...
                                style={{
                                  ...
                                  padding: "2px 7px",
                                  color:
                                    showCouncil === m.id ? AC.claude : T.tf,
                                  fontSize: 9,
                                  cursor: "pointer",
                                }}
                              >
                      ...
                      >
                        <div
                          style={{
                            height: "100%",
                            width: cur.pct,
                            background: `linear-gradient(90deg,${cur.color}66,${cur.color})`,
      ...
      {page === "keys" && !devUnlocked && (
        ...
        >
          <div style={{ fontSize: 36, fontWeight: 900, color: AC.claude }}>
            ◆
          </div>
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontSize: 11, color: T.ts }}>
              Insere o PIN para aceder às API Keys
            </div>
          </div>
          <div
            style={{
          ...
          >
            <input
              ...
              style={{
                background: T.s2,
                border: `1px solid ${pinErr ? "#ef4444" : T.b1}`,
                borderRadius: 12,
                padding: "10px 14px",
                color: T.tx,
                fontSize: 14,
          ...
          >
            PIN predefinido: 3004. Muda em localStorage("cortex-dev-pin").
          </div>
        </div>
      )}
      ...
      {page === "keys" && devUnlocked && (
        ...
        >
          ...
          >
            <h2
              style={{ margin: 0, fontSize: 14, fontWeight: 800, color: T.tx }}
            >
              Chaves API
            </h2>
            <button
          ...
          <p style={{ margin: 0, fontSize: 11, color: T.ts }}>
            Sem key o lobe usa Groq para simular.
          </p>
          {[
            {
              id: "grok",
            ...
            },
            {
              ...
              ph: "gsk_...",
              desc: "Grátis · llama-3.3-70b",
            },
            {
              id: "openai",
              label: "OpenAI",
              color: AC.openai || "#74aa9c",
            ...
            },
            {
              id: "llama",
              label: "Llama (Groq)",
              color: AC.llama || "#e879f9",
              link: "console.groq.com/keys",
              ph: "gsk_...",
              desc: "llama-4-scout via Groq",
            },
            {
              id: "mistral",
              label: "Mistral",
              color: AC.mistral || "#f97316",
              link: "console.mistral.ai/api-keys",
              ph: "...",
              desc: "mistral-large-latest",
            },
            {
              id: "genspark",
              label: "Genspark",
              color: AC.genspark,
              link: "www.genspark.ai/settings/api",
              ph: "gs-...",
              desc: "Multi-AI synthesis",
            },
            {
              id: "manus",
              label: "Manus",
        ...
        >
          ...
          >
            <div>
              ...
              <p style={{ margin: "2px 0 0", fontSize: 10, color: T.ts }}>
                Conhecimento acumulado do sistema
              </p>
            </div>
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
              {[
                [() => setShowSeed(true), AC.genspark, "Seed"],
                [() => setShowExport(true), AC.perp, "Export"],
                [() => setShowImport(true), AC.gemini, "Import"],
                [
                  () => {
                    if (confirm("Apagar toda a memória?")) {
                      setBrain(defaultBrain);
                      saveBrain(defaultBrain);
                      setBuf([]);
                    }
                  },
                  "#ef4444",
          ...
          >
            {[
              [brain.semantic.length, "Factos", AC.claude, "◆"],
              [brain.episodic.length, "Sessões", AC.gemini, "◈"],
              [brain.patterns.length, "Padrões", AC.grok, "◉"],
              [brain.sessions, "Total", AC.genspark, "◎"],
            ].map(([n, l, c, ic]) => (
              <div
                key={l}
                style={{
              ...
              >
                <div style={{ fontSize: 10, color: c, marginBottom: 2 }}>
                  {ic}
                </div>
                <div
                  style={{
                ...
                <div style={{ fontSize: 8, color: T.ts, marginTop: 2 }}>
                  {l}
                </div>
              </div>
            ))}
          ...
          {[
            {
              title: "Semântica",
              sub: "Factos e preferências",
              color: AC.claude,
              icon: "◆",
              items: brain.semantic
                .slice()
                .reverse()
                .map((x) => `[${x.tipo}] ${x.descricao}`),
            },
            {
              title: "Episódica",
              sub: "Resumos",
              color: AC.gemini,
              icon: "◈",
              items: brain.episodic.slice().reverse(),
            },
            {
              title: "Padrões",
              sub: "Comportamentos",
            ...
            >
              <div
                style={{
                  padding: "9px 13px",
                  borderBottom:
                    sec.items.length > 0 ? `1px solid ${T.b2}` : "none",
                  display: "flex",
                  alignItems: "center",
                  gap: 6,
                }}
              >
                <span style={{ color: sec.color, fontSize: 12 }}>
                  {sec.icon}
                </span>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: T.tx }}>
                  ...
                  <div style={{ fontSize: 9, color: T.ts }}>{sec.sub}</div>
                </div>
                <span
                  style={{
                    marginLeft: "auto",
                ...
                sec.items.map((it, i) => (
                  <div
                    ...
                    style={{
                      ...
                      color: T.ts,
                      borderBottom:
                        i < sec.items.length - 1 ? `1px solid ${T.b2}` : "none",
                      lineHeight: 1.5,
                    }}
                  >
                    • {it}
          ...
          {brain.lastReflect && (
            <div style={{ fontSize: 8, color: T.tf, textAlign: "center" }}>
              Última reflexão: {new Date(brain.lastReflect).toLocaleString()}
            </div>
          )}
        </div>
      )}
      ...
      {page === "settings" && (
        ...
        >
          <h2 style={{ margin: 0, fontSize: 14, fontWeight: 800, color: T.tx }}>
            Definições
          </h2>
          <div
            style={{
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 10, color: T.ts, marginTop: 1 }}>
                {THEMES[theme].emoji} {THEMES[theme].name} —{" "}
                {Object.keys(THEMES).length} temas
              </div>
            </div>
            <button onClick={() => setShowTP(true)} style={btn(T, AC.claude)}>
              Mudar
            </button>
          </div>
          <div
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 9, color: T.ts, marginTop: 1 }}>
                {
                  Object.values(keys).filter((k) => k?.trim().length > 10)
                    .length
                }{" "}
                de {Object.keys(keys).length} activas
              </div>
            </div>
            <button onClick={() => setPage("keys")} style={btn(T, AC.perp)}>
              Gerir chaves
            </button>
          </div>
          <div
          ...
          >
            ...
            {[
              ["◉", AC.grok, "Grok", "grok-3"],
              ["◈", AC.gemini, "Gemini", "gemini-2.5-flash"],
              ["◇", AC.perp, "Perplexity", "sonar-pro (via Groq)"],
              ["◎", AC.genspark, "Genspark", "simulado (via Claude)"],
              ["◍", AC.manus, "Manus", "simulado (via Claude)"],
              ["○", AC.openai || "#74aa9c", "OpenAI", "gpt-4o"],
              ["◐", AC.deepseek || "#4d9fff", "DeepSeek", "deepseek-chat"],
              ["◑", AC.llama || "#e879f9", "Llama", "llama-4-scout via Groq"],
              ["◒", AC.mistral || "#f97316", "Mistral", "mistral-large-latest"],
              [
                "◓",
                AC.nemotron || "#a3e635",
                "Nemotron",
              ...
              ["◆", AC.claude, "Claude", "claude-opus-4-6 — Juiz final"],
            ].map(([ic, c, t, d]) => (
              <div
                key={t}
                style={{
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 10, fontWeight: 600, color: T.tx }}>
                    {t}
                  </div>
                  <div style={{ fontSize: 8, color: T.ts, marginTop: 1 }}>
                    {d}
