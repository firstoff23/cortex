...
Sistema multi-agente React com 11 lobos em paralelo, council synthesis via Codex como juiz, memória SQLite e proxy Ollama local. Stack: React 18, Express, SQLite, Tailwind, shadcn/ui. Ambiente: Windows 11 + PowerShell.
...
const LOBES = [
  ...
  { id: 'Codex',        name: 'Codex',      color: '#d97706', active: true,  mode: 'normal', model: 'Codex-3-5-sonnet-20241022', type: 'judge' },
...
router.post('/council', async (req, res) => {
  ...
  const results = await Promise.allSettled(
    activeLobes
      .filter(id => id !== 'Codex') // Codex é o juiz, não participa na ronda
  ...
  // Codex como juiz: sintetiza as respostas
...
function routerDecide(query, availableLobes) {
  ...
  return availableLobes.filter(id => id !== 'Codex'); // sem match → todos exceto juiz
}
```
