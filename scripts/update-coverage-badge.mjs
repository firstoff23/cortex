...
const badgePath = new URL("../badges/coverage.json", import.meta.url);

const summary = JSON.parse(await readFile(summaryPath, "utf8"));
const coverage = Number(summary.total?.lines?.pct);

if (!Number.isFinite(coverage)) {
  throw new Error("Não foi possível determinar a cobertura de linhas.");
}
...
console.log(`Badge de cobertura atualizado: ${badge.message}`);
